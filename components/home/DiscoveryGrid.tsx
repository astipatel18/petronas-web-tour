...
const tracks = [
  ...
  { 
    title: "The Experience", 
    href: "/experience", 
    // 🚀 NEW VERIFIED STABLE LINK: Close-up of the Skybridge and towers
    img: "https://images.unsplash.com/photo-1595841055318-5089e080753d" 
  },
  { 
    title: "Admission", 
        ...
        <div className="grid md:grid-cols-3 gap-px bg-white/10 border border-white/10">
          {tracks.map((t) => (
            <Link 
              ...
              className="group relative h-137.5 overflow-hidden bg-black"
            >
              {/* 🛡️ Next.js Optimized Image with full parameters */}
              <Image 
                src={`${t.img}?auto=format&fit=crop&w=1200&q=80`} 
                alt={t.title}
                fill 
                sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" 
                className="object-cover opacity-40 grayscale group-hover:grayscale-0 group-hover:scale-110 transition-all duration-1000"
                priority={t.title === "Plan Your Visit"}
              />

              {/* Gradient Overlay */}
              <div className="absolute inset-0 bg-linear-to-t from-black via-black/20 to-transparent z-10" />

              {/* Content (Z-indexed to stay above the Image) */}
              <div className="absolute bottom-12 left-12 right-12 z-20">
                <h4 className="text-white text-3xl font-bold uppercase tracking-tight mb-4 leading-none">
                  {t.title}
                </h4>
