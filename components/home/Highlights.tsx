...
const features = [
  { 
    title: "Skybridge", 
    desc: "Connect with the horizon on level 41 and 42.", 
    // 🚀 FIXED: Verified stable ID specifically for the bridge connection
    // img: "https://images.unsplash.com/photo-1590518342263-547e271a53f9" 
    img: " https://images.unsplash.com/photo-1544918877-460635b6d13e"
  },
  { 
    title: "Observation Deck", 
    desc: "360 degree views of the golden triangle.", 
    // 🚀 FIXED: Verified stable ID for the towers at dusk
  ...
  return (
    <section className="py-24 bg-deep-slate">
      <Container>
        <div className="grid md:grid-cols-2 gap-8">
          {features.map(f => (
            <Card key={f.title} className="group cursor-pointer border-white/10 bg-black overflow-hidden">
              <div className="relative h-80 overflow-hidden">
                <Image 
                  // 🚀 Add optimization strings to ensure the URL is valid for the loader
                  src={`${f.img}?auto=format&fit=crop&q=80&w=1000`} 
                  alt={f.title} 
                  fill 
                  sizes="(max-width: 768px) 100vw, 50vw"
                ...
                {/* 🎨 Style Fix: Changed 'bg-linear-to-t' to 'bg-gradient-to-t' for Tailwind v3 compatibility */}
                <div className="absolute inset-0 bg-linear-to-t from-black via-black/40 to-transparent" />
                
                <div className="absolute bottom-8 left-8">
                  <h3 className="text-2xl font-bold text-white uppercase tracking-tighter">{f.title}</h3>
                  <p className="text-slate-400 text-sm max-w-xs mt-2">{f.desc}</p>
                </div>
              </div>
            </Card>
          ))}
