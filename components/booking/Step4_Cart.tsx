// // components/booking/Step_Cart.tsx
// "use client";
...
export default function Step4_Cart() {
  const { 
    ...
    setStep, 
    reset,
    totalPrice // 🚀 READ FROM STORE (Calculated in Step 3)
  } = useBookingStore();

  // 🛡️ Guard: If no tickets or price is 0, show empty state
  if (totalPrice === 0) {
    return (
      <div className="text-center py-32 bg-white rounded-3xl border border-dashed border-slate-200">
        <div className="bg-slate-50 w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <h3 className="text-xl font-bold text-slate-900">Your cart is empty</h3>
        <p className="text-slate-500 mt-2 mb-8">Please select a valid date and time to calculate pricing.</p>
        <button 
          onClick={() => setStep(1)} 
          className="bg-black text-white px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-cyan-600 transition-all"
        >
      ...
      <div className="mb-12 flex items-center justify-between px-2">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900">
            Review your <span className="text-slate-400 italic font-serif">Order</span>
          </h2>
          <p className="text-sm text-slate-500 mt-1">Step 4 of 6 — Confirmation</p>
        </div>
        <button 
          onClick={reset} 
          className="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-red-500 transition-colors"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="p-8 grow">
              <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-[9px] font-black uppercase tracking-widest rounded mb-3 block w-fit">
                Admission Slot Secured
              </span>
              <h3 className="text-2xl font-bold text-slate-900">{selectedAttraction?.name}</h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6">
                <div className="flex items-center gap-4">
                  <div className="p-2 bg-slate-50 rounded-lg"><Calendar size={18} className="text-cyan-600" /></div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Visit Date</span>
                ...
                <div className="flex items-center gap-4">
                  <div className="p-2 bg-slate-50 rounded-lg"><Clock size={18} className="text-cyan-600" /></div>
                  <div className="flex flex-col">
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">Entry Time</span>
          ...
          {/* Ticket Breakdown List */}
          <div className="space-y-4">
            <h4 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400 px-2">Visitor Details</h4>
            <div className="bg-white rounded-3xl border border-slate-100 p-8 space-y-6 shadow-sm">
              <TicketRow label="Adult" age="13-60 yrs" qty={tickets.adult} />
              <TicketRow label="Child" age="3-12 yrs" qty={tickets.child} />
              <TicketRow label="Senior" age="61+ yrs" qty={tickets.senior} />
              
              <div className="pt-6 border-t border-slate-50 flex items-center gap-3 text-slate-400 italic">
                <Info size={14} />
                <p className="text-[10px]">
                   Nationality Type: <span className="font-bold text-slate-600">{isMalaysian ? 'MyKad (Malaysian)' : 'Standard (International)'}</span>. Valid ID required at entrance.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-2xl shadow-slate-200/50">
            ...
            <div className="space-y-4 mb-10">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono font-bold">RM {totalPrice.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-500">Service & Processing</span>
                <span className="font-mono text-green-600 uppercase text-[10px] font-bold">Included</span>
              </div>
              <div className="h-px bg-slate-100 w-full" />
              <div className="flex justify-between items-end">
                <span className="text-slate-400 text-xs font-bold uppercase tracking-widest mb-1">Grand Total</span>
            ...
            <m.button 
              ...
              whileTap={{ scale: 0.98 }}
              onClick={() => setStep(5)}
              className="w-full bg-black text-white py-5 rounded-2xl font-bold uppercase text-[10px] tracking-[0.2em] shadow-xl hover:bg-cyan-600 transition-all flex items-center justify-center gap-3"
            >
              Continue to Details <ChevronRight size={14} />
            </m.button>

            <div className="mt-8 pt-8 border-t border-slate-50 space-y-3">
               <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-relaxed">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-500" /> 
                  Dynamic rates applied for chosen time & date
               </div>
            </div>
          </div>
        </div>
...
/** 
 * 🛠️ SUB-COMPONENT: TICKET ROW
 * We simply show the count and label. Since the pricing engine might add 
 * complex surges, showing the total RM at the bottom is clearer UX.
 */
function TicketRow({ label, age, qty }: { label: string, age: string, qty: number }) {
  if (qty === 0) return null;
  return (
    <div className="flex justify-between items-center group">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="font-bold text-slate-800">{label}</p>
          <p className="text-[10px] uppercase font-bold text-slate-400 tracking-tighter">{age}</p>
        </div>
      </div>
      <div className="bg-slate-50 px-3 py-1 rounded-lg font-mono text-sm font-bold text-slate-900 border border-slate-100">
        x{qty}
      </div>
    </div>
  );
